...
print(nac + labs(y="Number of active cells per recording", x = NULL,colour = "Genotype") +
        theme(
...
par(mar=c(5,5,5,4)+.1)
notBoxplot
notboxplot(data3,ylab="Number of active cells per recording",group.names= c("WT","KO"),cex.lab = 2.1, cex.axis = 2)



#2-t-test
t.test(dv~grp,data=data,alternative="two.sided")
#p-value =  0.1338, not significant difference
...
#p-value =  0.1209, not significant difference
...
par(mar=c(5,1,4,3)+.1)
a<-ggplot(A,aes(x=dv)) +
  geom_histogram(color=hue_pal()(4)[1], fill= hue_pal()(4)[1], alpha=0.5, breaks = seq(0,50,10))
...
#histogram for C=KO
c<-ggplot(C,aes(x=dv)) +
  geom_histogram(color=hue_pal()(4)[3], fill= hue_pal()(4)[3], alpha=0.5, breaks = seq(0,50,10))
